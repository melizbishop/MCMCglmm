...
LFlydat <- read.table("/Users/meganbishop/behavioral-evolution/LogBehaviorData.txt", header=TRUE, sep="\t", row.names="id")
...
MeansMCMC <- lapply(model0, function(m) m$Sol)
MeansMCMC <- do.call(mcmc.list, MeansMCMC)
MeanGelman=gelman.diag(MeansMCMC)

VarMCMC <- lapply(model0, function(m) m$VCV)
