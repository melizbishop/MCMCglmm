...
entropy_target_val <- 50 # Default value if not specified
entropy_intensity_val <- 1000 # Default value if not specified
use_entropy_penalty <- 1L # Integer flag: 0 = No, 1 = Yes

# Validate and set entropy_target_val
if (!is.null(entropy_target)) {
    if (!is.numeric(entropy_target) || length(entropy_target) != 1 || entropy_target <= 0) {
        stop("entropy_target must be a single positive numeric value.")
    }
    entropy_target_val <- as.double(entropy_target)
}

# Validate and set entropy_intensity_val
if (!is.null(entropy_intensity)) {
    if (!is.numeric(entropy_intensity) || length(entropy_intensity) != 1 || entropy_intensity < 0) {
        stop("entropy_intensity must be a single non-negative numeric value.")
    }
    entropy_intensity_val <- as.double(entropy_intensity)
}

# Final logic to determine if entropy penalty should be used
if (!is.null(entropy_target) && !is.null(entropy_intensity)) {
    use_entropy_penalty <- 1L
} else if (is.null(entropy_target) && is.null(entropy_intensity)) {
    # No action needed, already default
} else {
    warning("Both entropy_target and entropy_intensity should be specified if using an entropy penalty. Penalty will not be applied.")
    use_entropy_penalty <- 0L
    entropy_target_val <- 0.0
    entropy_intensity_val <- 0.0
}
# >>> END OF ENTROPY TERM BLOCK <<<
