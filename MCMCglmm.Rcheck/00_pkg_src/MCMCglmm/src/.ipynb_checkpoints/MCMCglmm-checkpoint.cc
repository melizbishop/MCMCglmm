...
#include <Rmath.h> // For lgammafn and digamma

extern "C"{  
    
double calculate_dirichlet_entropy(const double* log_alpha_values, int num_elements) {
    if (num_elements == 0) return 0.0; // Handle empty alpha vector

    double alpha0 = 0.0;
    // We'll also store the actual alpha values
    double* alpha_actual_values = new double[num_elements]; // Dynamically allocate

    // Calculate actual alpha values and their sum (alpha0)
    for (int i = 0; i < num_elements; ++i) {
        // Exponentiate to get actual alpha.
        alpha_actual_values[i] = std::exp(log_alpha_values[i]);

        // Basic check for valid alpha values (must be positive).
        if (alpha_actual_values[i] <= 1e-10) { // Using a small epsilon to avoid issues with zero/negative
            // For now, returning a large positive energy to strongly penalize invalid alpha.
            delete[] alpha_actual_values; // Free dynamically allocated memory
            return 1e30;
        }
        alpha0 += alpha_actual_values[i];
    }

    // Check for alpha0 being positive
    if (alpha0 <= 1e-10) {
        delete[] alpha_actual_values;
        return 1e30;
    }

    double sum_gammaln_alpha = 0.0;
    for (int i = 0; i < num_elements; ++i) {
        sum_gammaln_alpha += lgammafn(alpha_actual_values[i]);
    }

    double sum_alpha_minus_1_psi_alpha = 0.0;
    for (int i = 0; i < num_elements; ++i) {
        sum_alpha_minus_1_psi_alpha += (alpha_actual_values[i] - 1.0) * digamma(alpha_actual_values[i]);
    }

    double entropy = -(lgammafn(alpha0) - sum_gammaln_alpha + (alpha0 - num_elements) * digamma(alpha0) - sum_alpha_minus_1_psi_alpha);

    delete[] alpha_actual_values; // Free dynamically allocated memory
    return entropy;
}
    
double calculate_entropy_penalty(double target_entropy, double current_entropy, double intensity) {
    if (target_entropy == 0.0) { // Avoid division by zero
        return 1e30; // Large penalty for invalid target
    }
    double cost = intensity * pow(((current_entropy - target_entropy) / target_entropy), 2.0);
    return cost;
}
...
void MCMCglmm(
    ...
    int *verboseP, // print iterations and MH acceptance ratio to screen
    ...
    int *truncP, // should latent variables be truncated tp prevent under/overflow for cat/mult/thresh/ord distributions and zero-processes in zi/za/hu?
    ...
    double *entropy_target_ptr,
    double *entropy_intensity_ptr,
    int *use_entropy_penalty_ptr
){       
...
// --- START OF NEW VARIABLE DECLARATIONS AND DEREFERENCING ---
double entropy_target = *entropy_target_ptr;
double entropy_intensity = *entropy_intensity_ptr;
int use_entropy_penalty = *use_entropy_penalty_ptr;
// --- END OF NEW VARIABLE DECLARATIONS AND DEREFERENCING ---
     ...
     for(k=nG; k<nGR; k++){      // Iterate through R-structures
       ...
       for(j=0; j<nlGR[k]; j++){     // Iterate through levels
         ...
         if(mvtype[cnt+j]<0){
           ...
           double delta_energy_cost = 0.0;
           if (use_entropy_penalty == 1) { // Check if penalty is enabled
            // linki[k]->x and linki_tmp[k]->x are assumed to be your log_alpha_values
            // dimG is assumed to be the number of elements in the alpha vector for this block
               double current_entropy = calculate_dirichlet_entropy(linki[k]->x, dimG);
               double proposed_entropy = calculate_dirichlet_entropy(linki_tmp[k]->x, dimG);
               if (current_entropy > 1e29 || proposed_entropy > 1e29) {
                   delta_energy_cost = 1e30; // Force rejection if alpha values were invalid
               } else {
                     double energy_current = calculate_entropy_penalty(entropy_target, current_entropy, entropy_intensity);
                     double energy_proposed = calculate_entropy_penalty(entropy_target, proposed_entropy, entropy_intensity);
                     delta_energy_cost = energy_proposed - energy_current;
               }
           }
           ...
           if((densityl2 - densityl1 - delta_energy_cost) > log(runif(0.0,1.0))){
             for(i=0; i<dimG; i++){
